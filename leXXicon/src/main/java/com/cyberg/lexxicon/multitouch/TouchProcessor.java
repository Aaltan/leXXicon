...
public class TouchProcessor {
	...
	// heuristic constants
	public long TAP_INTERVAL = 200;
	public long TAP_TIMEOUT = 200;
	public int DOUBLE_TAP_DIST_THRESHOLD = 30;
	public int FLICK_VELOCITY_THRESHOLD = 20;
	public float MAX_MULTI_DRAG_DISTANCE = 100; // from the centroid

	// A list of currently active touch points
	public ArrayList<TouchPoint> touchPoints;

	// Used for tap/doubletaps
	public TouchPoint firstTap;
	public TouchPoint secondTap;
	public long tap;
	...
	// -------------------------------------------------------------------------------------
	public TouchProcessor(Main aFather) {
		mFather = aFather;
	...
	public synchronized void pointDown(float x, float y, int id) {
		...
		if (touchPoints.size() >= 2) {
			...
			if (touchPoints.size() == 2) {
				// if this is the second point, we now have a valid centroid to update
				// the first point
	...
	public synchronized void pointUp(int id) {
		...
		if (p == firstTap || p == secondTap) {
			...
			if (d > FLICK_VELOCITY_THRESHOLD) {
				FlickEvent event = new FlickEvent(p.px, p.py, new PVector(p.x - p.px,
						p.y - p.py));
				events.add(event);
			} 
			else {
				long interval = mFather.millis() - tap;
	...
	public synchronized void pointMoved(float x, float y, int id) {
		...
		// since the events will be in sync with draw(), we just wait until
		// analyse() to
		// look for gestures
		pointsChanged = true;
	}
	...
	public synchronized void analyse() {
		handleTaps();
		// simple event priority rule: do not try to rotate or pinch while
		// dragging
	...
	public void sendEvents() {
		for (int i = 0; i < events.size(); i++) {
			...
			if (e instanceof TapEvent) {
				mFather.onTap((TapEvent) e);
			}
			else if (e instanceof FlickEvent) {
				mFather.onFlick((FlickEvent) e);
			}
			else if (e instanceof DragEvent) {
				mFather.onDrag((DragEvent) e);
			}
			else if (e instanceof PinchEvent) {
				mFather.onPinch((PinchEvent) e);
			}
			else if (e instanceof RotateEvent) {
				mFather.onRotate((RotateEvent) e);
			}
		}
		events.clear();
	}
	...
	// -------------------------------------------------------------------------------------
	// pinch is simply the average distance change from each points to the
	// centroid
	public void handlePinch() {
		...
		if (pinch != 0) {
			events.add(new PinchEvent(cx, cy, pinch, touchPoints.size()));
		}
	}
	...
	public boolean handleDrag() {
		...
		// multi-drag is defined as all the fingers moving close-ish together in
		// the same direction
		boolean x_drag = true;
		boolean y_drag = true;
		...
		if ((x_drag || y_drag) && clustered) {
			if (touchPoints.size() == 1) {
				...
				// use the centroid to calculate the position and delta of this drag
				// event
	...
	// -------------------------------------------------------------------------------------
	public synchronized ArrayList<TouchPoint> getPoints() {
